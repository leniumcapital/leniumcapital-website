/**
 * Single source of truth for challenge pricing, rules constants, add-ons,
 * marketing data, and tier limits. Checkout uses `PricingTier` / `TIERS`;
 * the rules engine uses `Tier` / `TIERS_RULES` mapped from the same array.
 */
...
// ─── Rules tier (percent fields as whole numbers, e.g. 20 = 20%) ─────────────
...
export function pricingTierToRulesTier(t: PricingTier): Tier {
  return {
    size: t.size,
    baseFee: t.fee,
    resetFee: t.resetFee,
    profitTargetPct: Math.round(t.profitTarget * 100),
    maxDrawdownPct: Math.round(t.maxDrawdown * 100),
    maxPositionPct: Math.round(t.maxPositionSize * 100),
    maxExposurePct: Math.round(t.maxExposure * 100),
    windowDays: CHALLENGE_WINDOW_DAYS,
    consistencyCapPct: 15,
    ...(t.popular ? { featured: true } : {}),
    ...(t.exclusive ? { exclusive: true } : {}),
  };
}

/** Rules-engine tiers — derived from checkout `TIERS`, never edited separately. */
export const TIERS_RULES: Tier[] = TIERS.map(pricingTierToRulesTier);
...
export const resetLineLong = (tier: Tier) =>
  `Restart this challenge — ${formatUsd(tier.resetFee)} (save ${formatUsd(resetSavings(tier))} vs buying fresh)`;

export const resetLineShort = (tier: Tier) =>
  `Reset a failed attempt: ${formatUsd(tier.resetFee)}`;

export const resetCheckoutTitle = (tier: PricingTier | Tier) =>
  `${formatUsd(tier.size)} Challenge — Reset Purchase`;
...
export const TIER_COMPARISON_ROWS: RuleRow[] = [
  {
    label: "Profit target",
    format: (t) => `20% (${formatUsd(demoTargetUsd(t))})`,
    plain: "Uniform 20% across all six tiers.",
  },
  {
    label: "Max drawdown",
    format: (t) => `10% static (${formatUsd(staticDrawdownFloorUsd(t))} floor)`,
  ...
  },
  {
    ...
    format: (t) => `5% (${formatUsd(maxPositionUsd(t))})`,
    plain: "No single position may exceed 5% of current balance at entry.",
  },
  {
    label: "Max total exposure",
    format: (t) => `10% (${formatUsd(maxExposureUsd(t))})`,
...
export const RULE_ROWS: RuleRow[] = [
  ...
  },
  {
    label: "Max drawdown",
    format: (t) =>
      `10% static floor (${formatUsd(staticDrawdownFloorUsd(t))})`,
    plain:
...
export const FUNDED_RULE_ROWS: RuleRow[] = [
  ...
  },
  {
    ...
    format: (t) =>
      `5% / 10% of current balance (${formatUsd(maxPositionUsd(t))} / ${formatUsd(maxExposureUsd(t))} at start)`,
    plain:
  ...
  },
  {
    ...
    format: (t) => `2% of starting balance (${formatUsd(minPayoutUsd(t))})`,
...
/** Alias for marketing pages — same as formatUsd. */
export const usd = formatUsd;

/** Alias for marketing pages — same as formatTierCompact. */
export const compactTier = formatTierCompact;
