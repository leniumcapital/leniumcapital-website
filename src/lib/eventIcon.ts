...
function matchHardcoded(displayName: string, seriesTicker: string): HardcodedMatch | null {
  const hay = `${displayName} ${seriesTicker}`.toLowerCase();

  if (/fifa|world cup|soccer cup|wcgame|soccer/.test(hay)) {
    return { Icon: IconBallFootball, color: "#fff" };
  }
  if (/election|presidential|senate|congress|ballot|primary|primaries/.test(hay)) {
  ...
  if (/wnba|women.*basketball|basketball \(w\)/.test(hay)) {
    return { Icon: IconBallBasketball, color: "#fff" };
  }
  if (/nba|basketball|playoffs/.test(hay)) {
    return { Icon: IconBallBasketball, color: "#fff" };
  }
  if (/nfl|super bowl|pro football/.test(hay)) {
    return { Icon: IconBallAmericanFootball, color: "#fff" };
  }
  if (/mlb|world series|baseball|pro baseball/.test(hay)) {
    return { Icon: IconBallBaseball, color: "#fff" };
  }
  if (/nhl|hockey|pro hockey/.test(hay)) {
    return { Icon: IconBallVolleyball, color: "#fff" };
  }
  if (/ufc|mma|fight/.test(hay)) {
    return { Icon: IconKarate, color: "#fff" };
  }
  if (/champions league|uefa|premier league|tennis/.test(hay)) {
...
/** Resolve icon for a featured event shortcut card. */
export function resolveEventIcon(
  ...
  const hardcoded = matchHardcoded(displayName, seriesTicker);
  if (hardcoded) {
