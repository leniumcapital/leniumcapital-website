...
export function AccountModeSwitch() {
  ...
  const hasDemo = useAccountStore((s) => s.hasActiveChallenge);
  const hasLive = useAccountStore((s) => s.hasFundedAccount);
  ...
  async function switchTo(mode: Mode) {
    ...
    try {
      ...
      useAccountStore.getState().applyAccountStatus({
        ...
        hasActiveChallenge: hasDemo || storeType === "challenge",
        hasFundedAccount: hasLive || storeType === "funded",
        ...(nextMode === "demo"
          ? { challengeBalance: a.balance, challengeTier: a.tier }
          : { fundedBalance: a.balance, fundedTier: a.tier }),
      });
      setAccount({
